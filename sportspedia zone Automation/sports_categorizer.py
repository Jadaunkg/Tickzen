...
class SportsNewsCategorizer:
    def __init__(self, source_database: str = "sports_news_database.json"):
        ...
        self.categories = {
            'cricket': {
                'filename': 'cricket_news_database.json',
                'keywords': {
            ...
            'football': {
                'filename': 'football_news_database.json', 
                'keywords': {
                    # Football terms
            ...
            'basketball': {
                'filename': 'basketball_news_database.json',
                'keywords': {
                    # Basketball terms
    ...
    def categorize_article(self, article: Dict) -> str:
        """Categorize a single article based on title, summary, and source"""
    ...
    def categorize_all_articles(self, save_individual_files: bool = True) -> Dict:
        ...
        for category in self.categories.keys():
            categorized_data[category] = {
                ...
                'metadata': {
                    ...
                    'total_articles': 0,
                    'sources': [],
                    'parent_database': self.source_database
                }
            }
        ...
        self.save_category_database(categorized_data['uncategorized'], 'uncategorized_news_database.json')
        
        # Log results
        logging.info("Categorization Results:")
        for category, count in categorization_stats['categorized'].items():
            if count > 0:
                logging.info(f"  {category.capitalize()}: {count} articles")
        logging.info(f"  Uncategorized: {categorization_stats['uncategorized']} articles")
    ...
    def save_category_database(self, category_data: Dict, filename: str):
        """Save a category database to file"""
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                json.dump(category_data, f, indent=2, ensure_ascii=False)
            logging.info(f"Saved {len(category_data['articles'])} articles to {filename}")
